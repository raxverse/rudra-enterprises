...
export default function WhyChooseUs() {
  return (
    <section className="py-20 spice-gradient relative overflow-hidden">
      <div className="absolute top-0 right-0 w-72 h-72 rounded-full bg-orange-100/50 -translate-y-1/2 translate-x-1/2" />
      <div className="absolute bottom-0 left-0 w-48 h-48 rounded-full bg-amber-100/50 translate-y-1/2 -translate-x-1/2" />
      
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <span className="inline-block text-orange-600 text-sm font-semibold tracking-widest uppercase mb-3">Our Promise</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-7">
          {features.map((feature) => {
            ...
            return (
              <div
                key={feature.title}
                className={`${feature.bg} border ${feature.border} rounded-2xl p-7 card-hover text-center`}
              >
                <div className={`inline-flex items-center justify-center w-14 h-14 ${feature.bg} rounded-full mb-5 border-2 ${feature.border} shadow-sm`}>
                  <Icon className={`w-7 h-7 ${feature.color}`} />
                </div>
                <h3 className="text-lg font-bold text-amber-900 mb-3" style={{ fontFamily: "'Playfair Display', serif" }}>
                  {feature.title}
                </h3>
                <p className="text-amber-800/80 text-sm leading-relaxed">{feature.description}</p>
              </div>
            );
          })}
        </div>
